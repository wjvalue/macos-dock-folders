...
>    再 `dg apply`。
> 另外两条零闪规则（2026-09-22）：
> ① 配置没变时 apply 跳过 Dock 重启（不闪）；
> ② **纯图标变化**（换风格、加删成员 → bundle 重建但 Dock 条目不变）也跳过重启 ——
>    新版 icns 文件名保证**下次点击分组图标时立即显示新样式**。只有 Dock 条本身
>    变化（加删分组、改位置/布局/材质）才 killall Dock。别再往回加「图标变了就
>    强制重启」的逻辑 —— 那是老大明确砍掉的闪屏来源。
...
9. **拼贴图标在 64px 下的可读性**（踩过）：
   ...
   - ⚠️ 2026-09-22 按老大要求改为**全覆盖**：画布留白收到 **2%**（ICON_INSET=0.02）、
     内边距 **0.045**、间距 **0.035** —— 浅色风格在 Dock 上不再显出一圈留白边，
     拼贴里的 App 图标也更大。旧参数（留白 7.5%）仅存档在 pitfalls #7。
   - 注意：作为 **App** 放进 Dock 时，系统会把图标归一化到 86.5% 内容框，
     所以渲染结果与合成图会有像素差异 —— **这是正常的**，各风格一视同仁。
