...
> ⚠️ 2026-09-22 更新：老大要求「全覆盖、拼贴里的 App 图标更大」，留白收到
> **2%**、内边距 **0.045**、间距 **0.035**。上面的 0.44/0.085/0.045/7.5% 是
> 9-13 ~ 9-22 期间的历史参数，仅作考古用。
...
**现象**：`blendingMode = .behindWindow` 的毛玻璃会**实时采样窗口背后的内容**，
而面板又弹在任意窗口之上。背后左边是深色终端、右边是浅色桌面时，玻璃就左半边深、
右半边浅，像被劈成两半 —— 实测同一块面板内部亮度能从 50 平滑爬到 154。

**做法**：压一层半透明匀色底把自适应抹平。压完之后全宽亮度差 ≤3，任何背景下都是
同一块稳定的表面（`docs/panel-materials.png` 里 `hud` 那块就是压过的结果）。
...
**别踩的**：`appearance` 必须设在毛玻璃视图上，只设在 window 上无效；改了材质却看不到
变化时先怀疑这个。

## 19. 圆角必须一路裁到窗口层

**现象**：只给内容视图设圆角，四角会各露出一个直角块 —— 因为窗口自己的 theme frame
和背后的毛玻璃还是方的。

**做法**：圆角要一路往下设：theme frame 的 `contentView.layer` 圆角
＋ 毛玻璃层的 `maskImage` 用同一套圆角
＋ 改完调 `invalidateShadow()`，否则阴影还是按方角算。

实测对比图（左：只裁内容视图；右：裁到窗口层）：

![圆角：只裁内容视图 vs 裁到窗口层](images/panel-corner-radius.png)

> 这张是当时的原始截屏，画面里还带着终端内容，留作证据没做清理。
> README 里用的配图是脚本重新合成的，见 `tools/readme_assets.py`。
...
## 23. macOS 15+ 把「右键 → 打开」Gatekeeper 旁路砍了

**现象**（2026-09-22，macOS 26.6.2 实测）：分发的 `DockGroup.app` 首开被拦，
弹窗只有「完成」和「移到废纸篓」两个按钮 —— README 里写的「右键 → 打开」
在右键菜单里**根本不存在**。这条路 macOS 15（Sequoia）起就被移除了。

**做法**：文档统一改成按版本对号入座：
- macOS 14 及更早：右键 →「打开」→ 确认；
- macOS 15+：① 点掉弹窗后去 **系统设置 → 隐私与安全性**，底部「安全性」区
  出现「"DockGroup" 已被阻止」→ 点「仍要打开」→ 验证密码 / Touch ID；
  ② 或终端 `xattr -cr /path/to/DockGroup.app`（我们实测这条最省事）。
...
## 25. macOS 26「squircle 监狱」：自带 icns 被缩进白框，自定义图标豁免（2026-09-22）

**症状**：分组图标在 Dock 里缩小一圈 + 套白框，换任何风格都一样，预览图对不上。

**根因**：Tahoe 对 App **自带**的 icns 做 squircle 合规检查——底板弧外出现
中高不透明像素（实测 alpha >~150：hairline 居中描边溢出的半圈 210、图标角
擦出弧线的 201 都算）就把整张图缩进系统容器垫灰底，与底板占比无关
（96%/80% 一样缩）。带 Assets.car 的新式图标免疫，纯 icns 全中招。

**修法**（三件套，缺一不可）：
1. `NSWorkspace.setIcon(mosaic, forFile: app)` 把拼贴图设成**自定义图标**
   （唯一豁免通道）。⚠️ 必须压轴（codesign/lsregister/mtime 都会重置图标
   记录），且每轮无条件重设自愈。点目录 .apps 不是问题。
2. mosaic 生成端把图标+投影**裁进底板圆角**（角落格图标天然越出弧线）。
3. 收尾合规钳制：弧外 alpha 一律 ≤120（与 edge 描边同级，可通过）。
   不能用 `max(120, clip)`——hairline 骑在弧上，AA 过渡带会漏。

**后续**：豁免后图标被 1:1 渲染，全覆盖底板反而显大 → 拼贴图标改走
Apple 标准网格（`TILE_INSET = 0.098`），与 ICON_INSET（面板 96% 全覆盖）分家。
Python 引擎无法设自定义图标，已退役不补。
