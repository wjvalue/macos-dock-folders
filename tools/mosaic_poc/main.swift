// tools/mosaic_poc/main.swift
//
// make_mosaic() / _panel_base() 的 Swift 原型。
//
// 目的：验证「能不能用系统原生图形栈（CoreGraphics + CoreImage）做出和 Pillow
// 等效的分组图标」。这是全 Swift 化里最大的技术风险点 —— 其它部分（plist、
// JSON、文件系统、Process 调外部命令）在 Foundation 里都是一对一的。
//
// 几何常量和 STYLES 全部从 scripts/dockgroup.py 抄（不凭记忆），漂了就没意义。
//
// 要点：PIL 的语义和 CG 不完全一样，下面几处必须照抄 PIL 的行为，否则对比出来
// 的差异是假的：
...
//   /tmp/mosaic_poc <style> <size> <out.png> <icon1.png> [icon2.png ...]
...
let ICON_INSET = 0.075   // 画布四周留白
let BG_RADIUS = 0.235    // 文件夹底圆角
...
/// （PIL 内部是 3 次 box blur 近似，CI 是精确高斯，两者会有细微差别 ——
///  这正是本原型要量化的东西之一。）
...
func makeMosaicStyle(_ icons: [String], style: String, size S: Int, out: String) {
    ...
    print("✅ \(out)  (\(S)×\(S), style=\(style), \(n) 个图标, cell=\(cell))")
}
...
guard argv.count >= 4 else {
    FileHandle.standardError.write(
        "用法: mosaic_poc <style> <size> <out.png> <icon1.png> [icon2.png ...]\n".data(using: .utf8)!)
    exit(2)
}
let style = argv[0]
let size = Int(argv[1]) ?? 1024
let out = argv[2]
let icons = Array(argv[3...])
makeMosaicStyle(icons, style: style, size: size, out: out)
